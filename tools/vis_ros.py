...
class ROS_MODULE:
    ...
    def ros_print(self, pts, pred_dicts=None, last_box_num=None, gt_boxes=None, last_gtbox_num=None):
        ...
        if pred_dicts is not None:
            OriginalBoxes = pred_dicts[0]['pred_boxes']
            ...
            for obid in range(boxes.shape[0]):
                ...
                markert.text = '(' + str(np.floor(OriginalBoxes[obid][0] * 100) / 100)+ ', '+ str(np.floor(OriginalBoxes[obid][1] * 100) / 100)+ ', '+ str(np.floor(OriginalBoxes[obid][2] * 100) / 100) + ')'
